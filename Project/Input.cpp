...
bool Input::IsKeyPressedOnce(string name)
{
	static std::unordered_map<string, bool> keyWasPressed;
	auto it = pressedKeyMap.find(name);

	if (it != pressedKeyMap.end()) {
		// Check if the key is pressed
		bool isPressed = it->second;
		if (isPressed && !keyWasPressed[name]) {
			// Key is pressed and hasn't been pressed before, so register it
			keyWasPressed[name] = true;
			return true;
		}
		else if (!isPressed) {
			// Key is released, reset the state
			keyWasPressed[name] = false;
		}
	}
	return false;
}
